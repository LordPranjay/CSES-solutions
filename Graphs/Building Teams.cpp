...
vvi undig(int m,int n){ //m is edges and n is vertices, it assumes undirected edge between ff and ss
    vpii e(m);
    vvi adj(n);
    fr(i,0,m){
        cin>>e[i].ff>>e[i].ss;
        adj[e[i].ff-1].pb(e[i].ss-1);
        adj[e[i].ss-1].pb(e[i].ff-1);
    }
  return adj;
}
 
bool f(int i,vvi &adj,map<int,int> &mpp,vi &vis,int color){
    vis[i]=1;
    mpp[i]=color;
    for(auto j:adj[i]){
        if(vis[j]==1 && mpp[j]==color){
            return false;
        }
        else if(vis[j]==0){
        if (!f(j,adj,mpp,vis,3-color)){
            return false;
        }
        }
    }
